...
export default function FavoritesPage() {
  ...
  const [favItems, setFavItems] = useState<TrendItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetch('/api/trends')
      ...
      .then((data: TrendItem[]) => {
        setAllItems(data);
        setFavItems(data.filter(i => ids.has(i.id)));
      })
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 transition-colors">
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 space-y-3">
        {loading ? (
          ...
          <>
            <p className="text-xs text-gray-400 dark:text-slate-500 px-0.5">{favItems.length} 件のお気に入り</p>
            {favItems.map(item => (
